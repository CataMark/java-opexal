...
public class VendorServ {
    ...
    public static Optional<Vendor> getByCod (String cod, String userId) throws Exception{
        ParamSql[] parametri = new ParamSql[1];
        parametri[0] = new ParamSql(cod, Types.CHAR);
        
        return App.getConn(userId)
                .getFromPreparedStatement("select * from oxal1.fnc_vendor_get_by_cod(?);", Optional.of(parametri)).stream()
                .map(Vendor::new)
                .findFirst();
    }
    
    public static LazyDataModelRecords<Vendor> getLazyRecords(int after, int size, Optional<Map<String, String>> sort, Optional<Map<String, String>> filter, String userId) throws Exception{
        
        //pregatire baza sql
        String sqlBase = String.format(App.getSql("vendor_lazy_base"),
        ...
        CompletableFuture<List<Vendor>> fRecords = CompletableFuture.supplyAsync(() -> {
        ...
        LazyDataModelRecords<Vendor> rezultat = new LazyDataModelRecords<>();
        rezultat.setRecords(fRecords.get(30, TimeUnit.SECONDS));
        rezultat.setPozitii(fPozitii.get(30, TimeUnit.SECONDS));
        return rezultat;        
    }
    
    public static Optional<Vendor> insert(Vendor inreg, String userId) throws Exception{
        ...
        return App.getConn(userId)
                .getFromCallableStatement("{call oxal1.prc_vendor_insert_return(?,?,?)}", parametri).stream()
                .map(Vendor::new)
                .findFirst();
    }
    
    public static Optional<Vendor> update(Vendor inreg, String userId) throws Exception{
        Map<String, ParamSql> parametri = new HashMap<>();
        parametri.put("cod", new ParamSql(inreg.getCod(), Types.CHAR));
        parametri.put("nume", new ParamSql(inreg.getNume(), Types.NVARCHAR));
        parametri.put("kid", new ParamSql(userId, Types.VARCHAR));
        ...
        return App.getConn(userId)
                .getFromCallableStatement("{call oxal1.prc_vendor_update_return(?,?,?)}", parametri).stream()
                .map(Vendor::new)
                .findFirst();
    }
    ...
    public static boolean delete(String cod, String userId) throws Exception{
        ...
        return (boolean) App.getConn(userId)
                .getFromCallableStatement("{call oxal1.prc_vendor_delete_return(?)}", parametri).stream()
                .flatMap(x -> x.values().stream())
                .findFirst()
                .orElse(false);
    }
    ...
    public static void mergeLoad(UUID load_uuid, String userId) throws Exception{
        ...
        App.getConn(userId)
                .executeCallableStatement("{call oxal1.prc_vendor_merge_load(?)}", parametri);
    }
    
    public static void toXlsx(Optional<Map<String, String>> filter, String userId, OutputStream out) throws Exception{
        String sql = String.format(App.getSql("vendor_lazy_get_all_by_filter"), (filter.isPresent() ? getFilterSql(filter.get()) : ""));        
