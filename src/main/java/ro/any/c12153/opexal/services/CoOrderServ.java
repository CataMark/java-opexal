 ...
 * @author catalin
 */
public class CoOrderServ {
    
    public static Optional<CoOrder> getByCod (String cod, String userId) throws Exception{
        ParamSql[] parametri = new ParamSql[1];
        parametri[0] = new ParamSql(cod, Types.CHAR);
        
        return App.getConn(userId)
                .getFromPreparedStatement("select * from oxal1.fnc_co_order_get_by_cod(?);", Optional.of(parametri)).stream()
                .map(CoOrder::new)
                .findFirst();
    }
    
    public static LazyDataModelRecords<CoOrder> getLazyRecords(String coarea, int after, int size,
            Optional<Map<String, String>> sort, Optional<Map<String, String>> filter, String userId) throws Exception{
        
        //pregatire baza sql
        String sqlBase = String.format(App.getSql("co_order_lazy_base"),
        ...
        CompletableFuture<List<CoOrder>> fRecords = CompletableFuture.supplyAsync(() -> {
            List<CoOrder> result = new ArrayList<>();
            try {
        ...
        LazyDataModelRecords<CoOrder> rezultat = new LazyDataModelRecords<>();
        rezultat.setRecords(fRecords.get(30, TimeUnit.SECONDS));
        rezultat.setPozitii(fPozitii.get(30, TimeUnit.SECONDS));
        return rezultat;        
    }
    
    public static Optional<CoOrder> insert(CoOrder inreg, String userId) throws Exception{
        ...
        return App.getConn(userId)
                .getFromCallableStatement("{call oxal1.prc_co_order_insert_return(?,?,?,?,?,?,?)}", parametri).stream()
                .map(CoOrder::new)
                .findFirst();
    }
    
    public static Optional<CoOrder> update(CoOrder inreg, String userId) throws Exception{
        Map<String, ParamSql> parametri = new HashMap<>();
        parametri.put("cod", new ParamSql(inreg.getCod(), Types.CHAR));
        parametri.put("nume", new ParamSql(inreg.getNume(), Types.NVARCHAR));
        parametri.put("profit_center", new ParamSql(inreg.getPrctr(), Types.CHAR));
        ...
        return App.getConn(userId)
                .getFromCallableStatement("{call oxal1.prc_co_order_update_return(?,?,?,?,?)}", parametri).stream()
                .map(CoOrder::new)
                .findFirst();
    }
    ...
    public static boolean delete(String cod, String userId) throws Exception{
        ...
        return (boolean) App.getConn(userId)
                .getFromCallableStatement("{call oxal1.prc_co_order_delete_return(?)}", parametri).stream()
                .flatMap(x -> x.values().stream())
                .findFirst()
                .orElse(false);
    }
    ...
    public static void mergeLoad(UUID load_uuid, String userId) throws Exception{
        ...
        App.getConn(userId)
                .executeCallableStatement("{call oxal1.prc_co_order_merge_load(?)}", parametri);
    }
    ...
    public static void toXlsx(String coarea, Optional<Map<String, String>> filter, String userId, OutputStream out) throws Exception{
        String sql = String.format(App.getSql("co_order_lazy_get_all_by_filter"), (filter.isPresent() ? getFilterSql(filter.get()) : ""));
        ParamSql[] parametri = new ParamSql[1];
