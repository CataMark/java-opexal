...
public class CostCenter implements Serializable{    
    ...
    public CostCenter(JsonObject json){
        try {
            this.parseJson(json);
        } catch (Exception ex) {
            throw new RuntimeException(ex.getMessage(), ex.getCause());
    ...
    //pentru a merge cu primefaces autocomplete
    @Override
    public String toString() {
        if (Utils.stringNotEmpty(this.cod)){
            return this.cod
                    .concat(Utils.stringNotEmpty(this.nume) ? " ".concat(this.nume) : ""); 
        } else {
            return "";
        }
    }
