 ...
 * @author C12153
 */
@Named(value = "sapdoctip")
@ViewScoped
public class SapDocTypeItem implements Serializable, SelectItemView<SapDocType> {
    private static final long serialVersionUID = 1L;
    private static final Logger LOG = Logger.getLogger(SapDocTypeItem.class.getName());
    
    private @Inject @CurrentUser User cuser;
    private @Inject @CurrentLocale Locale clocale;
    
    private @Inject SelectTableView<SapDocType> owner;
    ...
    public void save(){
        try {
            if (this.selected == null || !Utils.stringNotEmpty(this.selected.getCod()))
                throw new Exception(App.getBeanMess("err.sapdoctip.nok", clocale));
            
            if (this.selected.getMod_timp() == null){
                SapDocType rezultat = SapDocTypeServ.insert(this.selected, cuser.getUname())
                ...
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
                    App.getBeanMess("title.sapdoctip.ins", clocale), App.getBeanMess("info.success",  clocale)));
                
            } else {
                SapDocType rezultat = SapDocTypeServ.update(this.selected, cuser.getUname())
                        .orElseThrow(() -> new Exception(App.getBeanMess("err.nosuccess", clocale)));
                ...
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
                    App.getBeanMess("title.sapdoctip.upd", clocale), App.getBeanMess("info.success",  clocale)));
            }
            ...
            FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_ERROR, App.getBeanMess("title.sapdoctip.save", clocale), ex.getMessage()));
        }
    }
    ...
    public void delete(){
        try {
            ...
            if (!SapDocTypeServ.delete(this.selected.getCod(), cuser.getUname()))
            ...
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
                    App.getBeanMess("title.sapdoctip.del", clocale), App.getBeanMess("info.success",  clocale)));
            if (Utils.stringNotEmpty(this.finishScript)) PrimeFaces.current().executeScript(this.finishScript);
            
        } catch (Exception ex) {
            App.log(LOG, Level.SEVERE, cuser.getUname(), ex);
            FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_ERROR, App.getBeanMess("title.sapdoctip.del", clocale), ex.getMessage()));
        }
    }
