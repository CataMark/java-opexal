...
@Named(value = "cocode")
@ViewScoped
public class CoCodeItem  implements Serializable, SelectItemView<CoCode> {
    private static final long serialVersionUID = 1L;
    private static final Logger LOG = Logger.getLogger(CoCodeItem.class.getName());
    
    private @Inject @CurrentUser User cuser;
    private @Inject @CurrentLocale Locale clocale;
    
    private @Inject SelectTableView<CoCode> owner;
    ...
    public void save(){
        try {
            ...
            if (this.selected.getMod_timp() == null){
                CoCode rezultat = CoCodeServ.insert(this.selected, cuser.getUname())
                ...
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
                    App.getBeanMess("title.cocode.ins", clocale), App.getBeanMess("info.success",  clocale)));
                
            } else {
                CoCode rezultat = CoCodeServ.update(this.selected, cuser.getUname())
                        .orElseThrow(() -> new Exception(App.getBeanMess("err.nosuccess", clocale)));
                ...
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
                    App.getBeanMess("title.cocode.upd", clocale), App.getBeanMess("info.success",  clocale)));
            }
            ...
            FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_ERROR, App.getBeanMess("title.cocode.save", clocale), ex.getMessage()));
        }
    }
    ...
    public void delete(){
        try {
            ...
            if (!CoCodeServ.delete(this.selected.getCod(), cuser.getUname()))
            ...
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
                    App.getBeanMess("title.cocode.del", clocale), App.getBeanMess("info.success",  clocale)));
            if (Utils.stringNotEmpty(this.finishScript)) PrimeFaces.current().executeScript(this.finishScript);
            
        } catch (Exception ex) {
            App.log(LOG, Level.SEVERE, cuser.getUname(), ex);
            FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_ERROR, App.getBeanMess("title.cocode.del", clocale), ex.getMessage()));
        }
    }
