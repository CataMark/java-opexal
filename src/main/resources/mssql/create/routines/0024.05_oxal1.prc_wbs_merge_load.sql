create or alter procedure oxal1.prc_wbs_merge_load
...
as
    begin
        ...
        begin try
            begin transaction
                merge into oxal1.tbl_int_wbs as t
                using (select * from oxal1.tbl_int_wbs_load as a
                        where a.load_id = @load_id) as s
                on (t.coarea = s.coarea and t.cod = s.cod)
                when matched then
                    update set
                        t.definitie = isnull(s.definitie, t.definitie),
                ...
                when not matched by target then
                    insert (cod, definitie, coarea, cocode, nume, profit_center, cost_center_resp, mod_de, mod_timp)
                    values (s.cod, s.definitie, s.coarea, s.cocode, s.nume, s.profit_center, s.cost_center_resp, s.mod_de, current_timestamp);
            commit transaction
        end try
        begin catch
