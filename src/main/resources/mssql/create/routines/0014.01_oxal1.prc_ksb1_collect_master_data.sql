create or alter procedure oxal1.prc_ksb1_collect_master_data
...
as
    begin
        set nocount on;
        begin try
            if not exists (select * from oxal1.tbl_int_ksb1 as a
                        where a.kokrs = @coarea and a.load_uuid = @load_uuid)
                return;

            /* prepare ksb1 temp table */
            select * into #ksb1
            ...
            using (select distinct a.kostl, a.obj_txt, a.kokrs, a.bukrs
                    from #ksb1 as a
                    where a.obj_txt is not null) as sursa
            on (tinta.coarea = sursa.kokrs and tinta.cod = sursa.kostl)
            when matched and sursa.obj_txt is not null then
                update set
            ...
            when not matched by target then
                ...
                values (sursa.kostl, sursa.obj_txt, sursa.kokrs, sursa.bukrs, @kid, current_timestamp);
        ...
        begin catch
            ...
            values ('oxal1.prc_ksb1_collect_master_data', 'ERROR', error_message(), @kid, current_timestamp);
        end catch
    end;